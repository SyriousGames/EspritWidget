...
class GameMath {
  ...
  /// Return true if [largerRect] entirely contains [smallerRect].
  static bool containsRect(Rect largerRect, Rect smallerRect) {
    return largerRect.contains(smallerRect.topLeft) &&
        largerRect.contains(smallerRect.bottomRight);
  }

  /// Return true if the rectangle bounded from [topLeft] to [topLeft] + [widthHeight]
  /// fits entirely inside of [rect].
  static bool fitsInsideRect(Rect rect, Offset topLeft, Offset widthHeight) {
    return rect.contains(topLeft) && rect.contains(topLeft + widthHeight);
  }
}
