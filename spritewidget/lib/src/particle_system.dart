...
class ParticleSystem extends Node {
  ...
  void _addParticle() {
    ...
    // Time to live
    particle.timeToLive =
        math.max(life! + lifeVar! * randomSignedDouble(), 0.0);
    ...
    // Rotation
    particle.rotation =
        startRotation! + startRotationVar! * randomSignedDouble();
    ...
    // Direction
    double dirRadians = convertDegrees2Radians(
        direction! + directionVar! * randomSignedDouble());
...
ParticleSystem deserializeParticleSystem(Map data,
  ...
  if (particleSystem == null) particleSystem = ParticleSystem(texture);
